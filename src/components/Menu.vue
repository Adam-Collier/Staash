<template>
    <div class="header">
        <Links />
        <div class="navbar">
            <svg :class="{close: isClosed}" @click="showAdd" viewBox="0 0 931 931" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" style="fill-rule:evenodd;clip-rule:evenodd;stroke-linecap:square;stroke-linejoin:round;stroke-miterlimit:1.41421;height:16px;"><g><path d="M465.131,47.631l0,835" style="fill:none;stroke-width:130px;stroke:#fff;"/><path d="M882.631,465.131l-835,0" style="fill:none;stroke-width:130px;stroke:#fff;"/></g></svg>
            <SubmitSite v-if="signedIn" :class="{reveal: isAdd}" :isAdd="isAdd" v-on:isAdd="isAdd = false, isClosed = false" v-on:message="message = $event"/>
            ...
            <input class="search" :class="{reveal: isSearch}" v-focus="isSearch" v-model="search" placeholder="Search sites" type="text" aria-label="Search" autocomplete="off" spellcheck="false">
            <div class="progress"></div>
            <Message v-bind="message" :class="{reveal: message.show}"/>
        </div>
    </div>
</template>
